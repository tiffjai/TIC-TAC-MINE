...
function processMove(game, row, col) {
  ...
  if (game.grid[row][col].isMine) return { valid: true, hitMine: true };
...
function checkGameStatus(game) {
  ...
  if (game.grid.every(row => row.every(cell => cell.revealed))) return { gameOver: true, winner: null }; // Draw
...
io.on('connection', (socket) => {
  ...
  socket.on('makeMove', ({ row, col, roomId }) => {
    ...
    if (!game || game.players[game.currentPlayer] !== socket.id) return;
    const result = processMove(game, row, col);
    if (result.valid) {
      io.to(roomId).emit('moveMade', { grid: game.grid, nextPlayer: game.currentPlayer });
      if (result.gameOver) io.to(roomId).emit('gameOver', { winner: result.winner, scores: game.scores });
    }
  });
  ...
  socket.on('disconnect', () => {
    for (let [roomId, game] of games) {
      if (game.players.includes(socket.id)) {
        ...
        if (!game.players.length) games.delete(roomId);
        break;
      }
    }
  });
